...
class Runner(object):
	...
	def load_data(self):
		...
		self.ent2img = torch.tensor(self.id2img)
		...
		self.ent2desc = torch.tensor(self.id2desc)
	...
	def construct_adj(self):
		...
		edge_index	= torch.LongTensor(edge_index).to(self.device).t() #2*len(triples)
	...
	def __init__(self, params):
		...
		if self.p.gpu != '-1' and torch.cuda.is_available():
			...
			torch.backends.cudnn.deterministic = True #固定的卷积算法
		else:
		...
		# self.optimizer,self.scheduler     = self.add_optimizer(self.model.parameters())
	...
	def add_model(self, model, score_func):
		...
		elif model_name.lower()	== 'mmkg_cp': 	model = CompGCN_CP(self.edge_index, self.edge_type, params=self.p,ent2img=self.ent2img,ent2desc=self.ent2desc)
	...
	def predict(self, split='valid', mode='tail_batch'):
		...
		with torch.no_grad():
			...
			for step, batch in enumerate(train_iter):
				sub, rel, obj, label	= self.read_batch(batch, split) #label ：B,ent_num
				pred			= self.model.forward(sub, rel)
				
				'''一下几行解释:
				首先,一个(dshead,rel),可能会有多个gold_tail, 为了避免其它tail的干扰,需要将其它的tail的pred_score排除在外
				本段代码采用的方式是将所有gold_tail的pred_score设置为非常小的值(负数)
				然后再把当前triple对应的gold_tail的pred_score恢复
				'''
				b_range			= torch.arange(pred.size()[0], device=self.device) #B
				target_pred		= pred[b_range, obj]
				pred 			= torch.where(label.byte(), -torch.ones_like(pred) * 10000000, pred) #所有标签不参与排序
	...
	def fit(self):
		...
		self.logger.info('MRR: {:.5}'.format(test_results["mrr"]))
		self.logger.info('MR: {:.5}'.format(test_results["mr"]))
		self.logger.info('Hits@1:  {:.5}'.format(test_results["hits@1"]))
		self.logger.info('Hits@3:  {:.5}'.format(test_results["hits@3"]))
		self.logger.info('Hits@10: {:.5}'.format(test_results["hits@10"]))
...
if __name__ == '__main__':
	...
	parser.add_argument('-num_workers',	type=int,               default=6,                     help='Number of processes to construct batches')
