...
def scatter_(name, src, index, dim_size=None):
	...
	# print(index.shape)
	# # exit()
	# print((index >40943).sum())
	# print((index <0).sum())

	# print("index.max(),index.min()",max(index),min(index))
	# exit()
	...
	def propagate(self, aggr, edge_index, **kwargs):
		...
		for arg in self.message_args:
			...
			if arg[-2:] == '_i':					# If arguments ends with _i then include indic
				...
				modality_weight = kwargs["modality_weight"][edge_index[0]]
				...
				modality_weight = kwargs["modality_weight"][edge_index[1]]
			...
			elif arg=='modality_weight':
				message_args.append(modality_weight)
...
class MessagePassingimg(torch.nn.Module):
	r"""Base class for creating message passing layers
	...
	def __init__(self, aggr='add'):
		super(MessagePassingimg, self).__init__()
